...
def check_default_optimizer(optimizer, model, prefix=''):
    ...
    if OPS_AVAILABLE:
        param_names = [
            ...
            'SyncBN.weight', 'SyncBN.bias', 'sub.param1', 'sub.conv1.weight',
...
def check_sgd_optimizer(optimizer,
    ...
    # SyncBN.weight
    SyncBN_weight = param_groups[4]
    assert SyncBN_weight['lr'] == base_lr
    assert SyncBN_weight['weight_decay'] == base_wd * norm_decay_mult
    # SyncBN.bias
    SyncBN_bias = param_groups[5]
    assert SyncBN_bias['lr'] == base_lr
    assert SyncBN_bias['weight_decay'] == base_wd * norm_decay_mult
    # sub.param1
...
def test_default_optimizer_constructor():
    ...
    # group 4, SyncBN is configured by 'norm_decay_mult'
    groups.append(['SyncBN.weight', 'SyncBN.bias'])
    group_settings.append({
        'lr': base_lr,
        'momentum': momentum,
    ...
    groups.append([
        ...
        'conv1.weight', 'conv2.weight', 'conv2.bias', 'SyncBN.weight', 'SyncBN.bias'
    ])
    group_settings.append({
        'lr': base_lr,
        'momentum': momentum,
