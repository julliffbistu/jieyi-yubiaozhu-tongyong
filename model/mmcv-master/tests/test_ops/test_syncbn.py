...
class TestSyncBN(object):
    ...
    def _test_SyncBN_train(self, size=1, half=False):
        ...
        mean = SyncBN.running_mean
        var = SyncBN.running_var
        if size == 1:
            x_grad = x.grad
            w_grad = SyncBN.weight.grad
            b_grad = SyncBN.bias.grad
        elif size == 2:
            x_grad = x.grad[rank % 2 * 4:rank % 2 * 4 + 4]
            w_grad = SyncBN.weight.grad / 2
            b_grad = SyncBN.bias.grad / 2
        elif size == 4:
            x_grad = x.grad[rank * 4:rank * 4 + 4]
            w_grad = SyncBN.weight.grad / 4
            b_grad = SyncBN.bias.grad / 4
