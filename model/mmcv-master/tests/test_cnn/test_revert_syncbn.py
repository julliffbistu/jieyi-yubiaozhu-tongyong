...
def test_revert_mmSyncBN():
    ...
    conv = revert_sync_batchnorm(conv)
    y_SyncBN = conv(x).detach().cpu().numpy()
    assert np.all(np.isclose(y_SyncBN, y_mmSyncBN, 1e-3))
    conv, x = conv.to('cpu'), x.to('cpu')
    y_SyncBN_cpu = conv(x).detach().numpy()
    assert np.all(np.isclose(y_SyncBN, y_SyncBN_cpu, 1e-3))
