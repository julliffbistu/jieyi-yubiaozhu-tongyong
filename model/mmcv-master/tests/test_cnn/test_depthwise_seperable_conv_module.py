...
def test_depthwise_separable_conv():
    ...
    # test dw_norm_cfg
    conv = DepthwiseSeparableConvModule(3, 8, 2, dw_norm_cfg=dict(type='SyncBN'))
    assert conv.depthwise_conv.norm_name == 'SyncBN'
    assert not conv.pointwise_conv.with_norm
    x = torch.rand(1, 3, 256, 256)
    output = conv(x)
    assert output.shape == (1, 8, 255, 255)
    ...
    # test pw_norm_cfg
    conv = DepthwiseSeparableConvModule(3, 8, 2, pw_norm_cfg=dict(type='SyncBN'))
    assert not conv.depthwise_conv.with_norm
    assert conv.pointwise_conv.norm_name == 'SyncBN'
    x = torch.rand(1, 3, 256, 256)
    output = conv(x)
    assert output.shape == (1, 8, 255, 255)

    # test norm_cfg
    conv = DepthwiseSeparableConvModule(3, 8, 2, norm_cfg=dict(type='SyncBN'))
    assert conv.depthwise_conv.norm_name == 'SyncBN'
    assert conv.pointwise_conv.norm_name == 'SyncBN'
    x = torch.rand(1, 3, 256, 256)
    output = conv(x)
