...
void sync_SyncBN_forward_mean_cuda(const Tensor input, Tensor mean) {
  SyncBNForwardMeanCUDAKernelLauncher(input, mean);
}

void sync_SyncBN_forward_var_cuda(const Tensor input, const Tensor mean,
                              Tensor var) {
  SyncBNForwardVarCUDAKernelLauncher(input, mean, var);
}

void sync_SyncBN_forward_output_cuda(const Tensor input, const Tensor mean,
...
void sync_SyncBN_backward_param_cuda(const Tensor grad_output, const Tensor norm,
                                 Tensor grad_weight, Tensor grad_bias) {
  SyncBNBackwardParamCUDAKernelLauncher(grad_output, norm, grad_weight,
                                        grad_bias);
}

void sync_SyncBN_backward_data_cuda(const Tensor grad_output, const Tensor weight,
                                const Tensor grad_weight,
...
REGISTER_DEVICE_IMPL(sync_SyncBN_forward_mean_impl, CUDA,
                     sync_SyncBN_forward_mean_cuda);
REGISTER_DEVICE_IMPL(sync_SyncBN_forward_var_impl, CUDA, sync_SyncBN_forward_var_cuda);
REGISTER_DEVICE_IMPL(sync_SyncBN_forward_output_impl, CUDA,
                     sync_SyncBN_forward_output_cuda);
REGISTER_DEVICE_IMPL(sync_SyncBN_backward_param_impl, CUDA,
                     sync_SyncBN_backward_param_cuda);
REGISTER_DEVICE_IMPL(sync_SyncBN_backward_data_impl, CUDA,
                     sync_SyncBN_backward_data_cuda);
