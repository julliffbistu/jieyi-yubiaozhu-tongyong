...
#include "sync_SyncBN_cuda_kernel.cuh"
...
void SyncBNForwardMeanCUDAKernelLauncher(const Tensor input, Tensor mean) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.scalar_type(), "sync_SyncBN_forward_mean_cuda_kernel", [&] {
        sync_SyncBN_forward_mean_cuda_kernel<scalar_t>
...
void SyncBNForwardVarCUDAKernelLauncher(const Tensor input, const Tensor mean,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.scalar_type(), "sync_SyncBN_forward_mean_cuda_kernel", [&] {
        sync_SyncBN_forward_var_cuda_kernel<scalar_t>
            <<<channels, THREADS_PER_BLOCK, 0, stream>>>(
...
void SyncBNForwardOutputCUDAKernelLauncher(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.scalar_type(), "sync_SyncBN_forward_mean_cuda_kernel", [&] {
        sync_SyncBN_forward_output_cuda_kernel<scalar_t>
            <<<channels, THREADS_PER_BLOCK, 0, stream>>>(
                input.data_ptr<scalar_t>(), mean.data_ptr<float>(),
...
void SyncBNBackwardParamCUDAKernelLauncher(const Tensor grad_output,
  ...
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad_output.scalar_type(), "sync_SyncBN_backward_param_cuda_kernel", [&] {
        sync_SyncBN_backward_param_cuda_kernel<scalar_t>
            <<<channels, THREADS_PER_BLOCK, 0, stream>>>(
...
void SyncBNBackwardDataCUDAKernelLauncher(const Tensor grad_output,
  ...
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad_output.scalar_type(), "sync_SyncBN_backward_data_cuda_kernel", [&] {
        sync_SyncBN_backward_data_cuda_kernel<scalar_t>
